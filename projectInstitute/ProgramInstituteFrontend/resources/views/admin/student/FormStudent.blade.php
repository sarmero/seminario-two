<div class="row g-3">
    ...
    <div class="col-sm-4">
        ...
        <input type="mail" class="form-control" name="mail" id="mail" placeholder=""
            value="@isset($person){{ old('mail', $person->email) }}@else{{ old('mail') }}@endisset">
    ...
    <div class="col-sm-4">
        ...
        <input type="text" class="form-control" name="phone" id="phone" placeholder=""
            value="@isset($person){{ old('phone', $person->phone) }}@else{{ old('phone') }}@endisset">
    ...
    <div class="col-sm-3">
        ...
        <select class="form-select" name="district" id="district" required alt="gandalf">
            ...
            @foreach ($district as $dis)
                <option value="{{ $dis->id }}" @if (old('district') == '' && isset($student)) {{ $person->district->id == $dis->id ? 'selected' : '' }}
                    @else
    ...
    <div class="col-sm-3">
        ...
        <input type="text" class="form-control" name="document" id="document" placeholder=""
            value="@isset($person){{ old('document', $person->document) }}@else{{ old('document') }}@endisset">
    ...
    <div class="col-sm-12 imageFile">
        <div class="mb-3 d-flex justify-content-center">
            <img name="image" id="preview-image-before-upload"
                src="@isset($person)
                {{ asset('storage/pofile/' . $person->image) }}
            @else
