<div class="row g-3">
    <div class="col-sm-12">
        ...
        <select class="form-select" name="program" id="program" required alt="gandalf">
            ...
            @foreach ($program as $pro)
                <option
                    value="{{ $pro->id }}" @if (old('program') == '' && isset($teacher)) {{ $teacher->program->id == $pro->id ? 'selected' : '' }}
                @else
    ...
    <div class="col-sm-6">
        ...
        <input type="text" class="form-control" name="firstName" id="firstName" placeholder=""
            value="@isset($teacher){{ old('firstName', $teacher->person->first_name) }}@else{{ old('firstName') }}@endisset">
    ...
    <div class="col-sm-6">
        ...
        <input type="text" class="form-control" name="lastName" id="lastName" placeholder=""
            value="@isset($teacher){{ old('lastName', $teacher->person->last_name) }}@else{{ old('lastName') }}@endisset">
        @error('lastName')
            <div class="alert alert-danger">{{ $message }}</div>
        @enderror
    </div>
    ...
    <div class="col-sm-4">
        ...
        <input type="mail" class="form-control" name="mail" id="mail" placeholder=""
            value="@isset($teacher){{ old('mail', $teacher->person->email) }}@else{{ old('mail') }}@endisset">
    ...
    <div class="col-sm-4">
        ...
        <select class="form-select" name="gender" id="gender" required alt="gandalf">
            ...
            <option value="M" @if (old('gender') == '' && isset($teacher)) {{ $teacher->person->gender == 'M' ? 'selected' : '' }} @else
                {{ old('gender') == 'M' ? 'selected' : '' }} @endif>Masculino</option>
            <option value="F" @if (old('gender') == '' && isset($teacher)) {{ $teacher->person->gender == 'F' ? 'selected' : '' }} @else
    ...
    <div class="col-sm-4">
        ...
        <input type="text" class="form-control" name="phone" id="phone" placeholder=""
            value="@isset($teacher){{ old('phone', $teacher->person->phone) }}@else{{ old('phone') }}@endisset">
    ...
    <div class="col-sm-3">
        ...
        <select class="form-select" name="district" id="district" alt="gandalf">
            ...
            @foreach ($district as $dis)
                <option value="{{ $dis->id }}" @if (old('district') == '' && isset($teacher)) {{ $teacher->person->district->id == $dis->id ? 'selected' : '' }}
                    @else
    ...
    <div class="col-sm-3">
        ...
        <input type="text" class="form-control" name="document" id="document" placeholder=""
            value="@isset($teacher){{ old('document', $teacher->person->document) }}@else{{ old('document') }}@endisset">
    ...
    <div class="col-sm-12 imageFile">
        <div class="mb-3 d-flex justify-content-center">
            <img name="image" id="preview-image-before-upload"
                src="@isset($teacher)
                {{ asset('storage/pofile/' . $teacher->person->image) }}
            @else
